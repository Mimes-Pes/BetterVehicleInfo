...
// Main mod.
namespace BetterVehicleInfo  // Name of the mod.
{
    ...
    {
        public Options() : base("Better Vehicle Info")
        {
        ...
        public static void Postfix(uGUI_SeamothHUD __instance)
        {
            ...
            if (main != null)
            {
                ...
                if (thisSeamoth != null)
                {
                    ...
                    bool upgradeLoaded = thisSeamoth.modules.GetCount(Modules.BetterVehicleInfoModule.TechTypeID) > 0;
    ...
    internal class SeaMoth_Update_Patch
    {
        ...
        public static void Postfix(SeaMoth __instance)
        {
            if (__instance.GetPilotingMode())
            {
                ...
                bool upgradeLoaded = __instance.modules.GetCount(Modules.BetterVehicleInfoModule.TechTypeID) > 0;
    ...
    {
        internal static bool playerDefaultValuesSet = false;
        internal static bool playerSwimsSlow = false;
        internal static bool playerSwimsNormal = false;
        internal static bool playerIsSprinting = false;
        //internal static float forwardMaxSpeed;
        //internal static float verticalMaxSpeed;
        //internal static float backwardMaxSpeed;
        //internal static float strafeMaxSpeed;
    ...
    internal class Vehicle_Update_Patch
    {
        ...
        public static void Postfix(Vehicle __instance)
        {
            ...
            if (main != null && thisSeamoth != null)
            {
                ...
                EnergyMixin thisEnergyMixing = thisSeamoth.GetComponent<EnergyMixin>();
                //float thisEnginePowerRating = Traverse.Create(__instance).Field("enginePowerRating").GetValue<float>();                

                if (upgradeLoaded && playerPiloting)
                {
                    bool modifyCharge = KeyCodeUtils.GetKeyHeld(KeyCode.W) || KeyCodeUtils.GetKeyHeld(KeyCode.S) || KeyCodeUtils.GetKeyHeld(KeyCode.A) || KeyCodeUtils.GetKeyHeld(KeyCode.D) || KeyCodeUtils.GetKeyHeld(KeyCode.C) || KeyCodeUtils.GetKeyHeld(KeyCode.Space);

                    if (Config.SeamothGearValue == 1f)
                    {
                        // execute if any of the movement keys are down
                        if (modifyCharge)
                            thisEnergyMixing.AddEnergy(0.0003f); // Charge change - net effect is to drain the cell slower
                    ...
                    {
                    if (modifyCharge)
                        thisEnergyMixing.AddEnergy(0.0002f); // Charge change
                    ...
                    {
                        if (modifyCharge)
                            thisEnergyMixing.AddEnergy(0.0001f); // Charge change
                    ...
                    {
                        if (modifyCharge)
                            thisEnergyMixing.ConsumeEnergy(0.001f); // Charge change
                    ...
                    {
                        if (modifyCharge)
                            thisEnergyMixing.ConsumeEnergy(0.0025f); // Charge change
                ...
                {
                    //if (__instance.forwardForce != 13f)
    ...
    internal class Player_Update_Patch
    {
        ...
        public static void Postfix(Player __instance)
        {
            if (__instance != null)
            {
                ...
                if (thisSeamoth != null)
                {
                    // var count = thisSeamoth.modules.GetCount(Modules.BetterVehicleInfoModule.TechTypeID);
                    bool upgradeLoaded = thisSeamoth.modules.GetCount(Modules.BetterVehicleInfoModule.TechTypeID) > 0;
                    ...
                    if (upgradeLoaded && playerPiloting)
                    {
                        ...
                        //float thisEnginePowerRating = Traverse.Create(thisSeamoth).Field("enginePowerRating").GetValue<float>();

                        //string forwardForceText = " " + __instance.GetVehicle().forwardForce.ToString(); // + " " + Player.main.GetMode().ToString();                       

                        // gear 1 to 4 (4 being default speed) is displayed white, gear 5 yellow, and gear 6 red ad energy consumption is high
                ...
                //float amount = 2f;
                //PlayerMotor activePlayerMotor = __instance.GetActivePlayerMotor();
               
                // just for testing, change the speed when swimming
                /*
                if (true)
                {

                    // load game speed defaults
                    PlayerInfo.forwardMaxSpeed = activePlayerMotor.forwardMaxSpeed;
                    PlayerInfo.verticalMaxSpeed = activePlayerMotor.verticalMaxSpeed;
                    PlayerInfo.backwardMaxSpeed = activePlayerMotor.backwardMaxSpeed;
                    PlayerInfo.strafeMaxSpeed = activePlayerMotor.strafeMaxSpeed;
                    PlayerInfo.acceleration = activePlayerMotor.acceleration;

                    if (KeyCodeUtils.GetKeyDown(KeyCode.Keypad1)) // go slower
                    {
                        activePlayerMotor.forwardMaxSpeed -= amount;
                        activePlayerMotor.verticalMaxSpeed -= amount;
                        activePlayerMotor.backwardMaxSpeed -= amount;
                        activePlayerMotor.strafeMaxSpeed -= amount;
                        activePlayerMotor.acceleration -= amount;
                    }
                    if (KeyCodeUtils.GetKeyDown(KeyCode.Keypad7)) // go faster
                    {
                        activePlayerMotor.forwardMaxSpeed += amount;
                        activePlayerMotor.verticalMaxSpeed += amount;
                        activePlayerMotor.backwardMaxSpeed += amount;
                    ...
                    if (KeyCodeUtils.GetKeyDown(KeyCode.Keypad4)) // reset to default
                    {
                        activePlayerMotor.forwardMaxSpeed = PlayerInfo.forwardMaxSpeed;
                        activePlayerMotor.verticalMaxSpeed = PlayerInfo.verticalMaxSpeed;
                        activePlayerMotor.backwardMaxSpeed = PlayerInfo.backwardMaxSpeed;
                        activePlayerMotor.strafeMaxSpeed = PlayerInfo.strafeMaxSpeed;
                        activePlayerMotor.acceleration = PlayerInfo.acceleration;
                    }
...
namespace BetterVehicleInfo.Modules
{
    public class BetterVehicleInfoModule : Equipable
    {
        public static TechType TechTypeID { get; protected set; }
        public BetterVehicleInfoModule() : base("BetterVehicleInfoModule", "Better Vehicle Info Module", "Enhances vehicle Health, Power and Temperature displays")
        {
            OnFinishedPatching += () =>
            {
